<template>
    <div class="h-screen rounded-lg bg-gray-300 flex w-full gap-4">
     <Navigation/>
...
const currentUserEmail = page.props.auth.user.email; // Get the current user's email
function onDragOver() {
    dragOver.value = true
}
...
function handleDrop(ev) {
    ...
    // Check user permission before uploading
    if (currentUserEmail === 'rimolcu.ac.24@gmail.com') {
        uploadFiles(files);
    } else {
        showErrorDialog('You are not allowed to upload the file');
    }
}
