...
<!--            <div><DeleteFilesButton :delete-all="allSelected" :delete-ids="selectedIds" @delete="onDelete"/>-->
<!--            </div>-->
        </nav>
        <div class="flex-1 overflow-auto">
            ...
            <table class="min-w-full">
                ...
                <tr v-for="file of allFiles.data" :key="file.id"
                    ...
                    <td class="px-6 bg-gray-300 py-4 whitespace-nowrap text-sm font-medium text-gray-900 w-[30px] max-w-[30px] pr-0">
                        <Checkbox @change="$event => onSelectCheckboxChange(file)" v-model="selected[file.id]" :checked="selected[file.id] || allSelected" />
                    </td>
                    <td class="px-6 bg-gray-300 py-4 whitespace-nowrap text-sm font-medium text-gray-900 flex items-center">
                        <FileIcon :file="file"/>
                        {{ file.name }}
                    </td>
                    <td class="px-6 py-4 bg-gray-300 whitespace-nowrap text-sm font-medium text-gray-900">
                        {{ file.path }}
                    </td>
                </tr>
                </tbody>
            ...
            <div v-if="!allFiles.data.length" class="py-8 text-center text-sm text-gray-900">
...
// Computed
const selectedIds = computed(() => Object.entries(selected.value).filter(a =>
    a[1]).map(a => a[0]))
